...
namespace lambhootDiscordBot
{
    class PartialBiGram
    {
        ...
        public string generateNewSentence(string input = null)
        {
            ...
            string returnString = "";
            int sentenceLength = (int)randomDoubleRange(minSentenceLength, maxSentenceLength * 0.6);
                ...
                else
                {
                    if (sentence.Count() > 0)//possibility that sentence has failed on first try, in which case, don't do this
                    {
                        if (!Char.IsPunctuation(returnString.Last()))
                            returnString += (randomDoubleRange(0, 100) > 50) ? "," : ".";
        ...
        public string generateNewBiGramSentence(string input = null)
        {
            ...
            int allSentenceLength = (int)randomDoubleRange(minSentenceLength, Math.Min(maxSentenceLength * 0.6, 25));
            while (CountWordsOfSentences(sentences) < allSentenceLength)
            {
                int sentenceLength = (int)randomDoubleRange(minSentenceLength, allSentenceLength);
        ...
        {
            int index = (int)randomDoubleRange(1, vocabulary.Count());//skips first testing word
            return vocabulary.ElementAt(index).Value;
        }

        public static float randomProbabilityForSentence()
        {
            float prob = (float)randomDoubleRange(minVocabWordProb * 0.2, maxVocabWordProb * 1.2);
            return prob;
        }
        ...
        public int selectRandomIndex(List<IndexProbPair> list)
        {
            ...
            while (index == 0)
            {
                index = (int)randomDoubleRange(0, list.Count());
